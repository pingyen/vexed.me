...
export default function Client() {
      ...
      fileReader.addEventListener('load', e => {
        ...
        image.addEventListener('load', function() {
          ...
          anchor.download = (() => {
            ...
            return `${prefix}.${format === 'jpeg' ? 'jpg' : format}`;
          })();
  ...
  return (
    <main>
      <h1 className="m-3 text-3xl font-bold">一次轉換多張 SVG 為 JPG 、 PNG 或 WebP</h1>
      <form onSubmit={formSubmit} className="text-2xl [&>div]:m-3 [&>div>span]:mr-2 [&>div>input]:my-2 [&>div>input]:border [&>div>input]:border-black [&>div>input]:p-2">
        <div><input type="file" name="files" accept="image/svg+xml" multiple required className="max-w-full" /></div>
        <div>
          <span>寬 (可省略)</span>
